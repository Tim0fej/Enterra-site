...
import { isSafeAttachmentUrl } from '../utils/safePath';
...
function MessageAttachment({ attachment }: { attachment: Attachment }) {
  ...
  }, [attachment.url, useAuthFetch]);

  const handleMediaError = () => {
    if (!useAuthFetch) {
      setUseAuthFetch(true);
      return;
    }
    setFailed(true);
  };
  ...
  if (failed) {
    return (
      <a href={attachment.url} className="message-attachments__fallback" target="_blank" rel="noopener noreferrer">
  ...
  if (isImageMime(attachment.mimeType)) {
    return (
      <a href={src} target="_blank" rel="noopener noreferrer" className="message-attachments__link">
        <img
          src={src}
