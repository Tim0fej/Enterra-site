...
/** Бейдж роли на сайте (админ / модератор) — PNG вместо текста */
export function SiteRoleBadge({
  role,
  size = 'sm',
  className = '',
}: {
  role: UserRole | string;
  size?: GroupBadgeProps['size'];
  className?: string;
}) {
  if (!isStaff(role as UserRole)) return null;
  return <GroupBadge role={role} size={size} className={className} />;
}
