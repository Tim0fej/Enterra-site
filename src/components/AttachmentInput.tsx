...
import { getToken, uploadAttachments } from '../api/client';
import { useLayoutError } from './Layout';
import { formatApiError } from '../utils/formatError';
import type { Attachment } from '../../shared/attachments';
import { ATTACHMENT_UPLOAD_HINT, MAX_ATTACHMENTS_PER_MESSAGE } from '../../shared/attachments';
...
export function AttachmentInput({ attachments, onChange, disabled = false }: AttachmentInputProps) {
  ...
  const handleFiles = async (fileList: FileList | null) => {
    ...
    if (remaining <= 0) {
      showError(`Не больше ${MAX_ATTACHMENTS_PER_MESSAGE} файлов`);
      return;
    }
    ...
    try {
      ...
      showError(formatApiError(err, 'Не удалось загрузить'));
    } finally {
      setUploading(false);
    ...
    <div className="attachment-input">
      ...
      {attachments.length < MAX_ATTACHMENTS_PER_MESSAGE && (
        <>
          ...
          <p className="attachment-input__hint muted">{ATTACHMENT_UPLOAD_HINT}</p>
        </>
      )}
    </div>
  );
