...
## Текущий production (Sprintbox)

| Параметр | Значение |
|----------|----------|
| VPS | `193.222.62.4` (purple-ubuntu2604-spb-02) |
| Домен | `https://enterra.tech` |
| Путь | `/opt/enterra-site` |
| Данные | `/var/lib/enterra/data` |
| Деплой | `DEPLOY_PASSWORD=... python deploy/upload-app.py` |

После смены A-записи DNS (`enterra.tech`, `www`) на `193.222.62.4`:

```bash
DEPLOY_HOST=193.222.62.4 DEPLOY_PASSWORD=... python deploy/setup-domain.py
```

Миграция: `OLD_DEPLOY_PASSWORD=... NEW_DEPLOY_PASSWORD=... python deploy/migrate-new-box.py`
